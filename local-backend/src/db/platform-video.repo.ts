...
export const platformVideoRepo = {
  ...
  upsert(data: {
    ...
    if (!existing) {
      db.prepare(`
        INSERT INTO platform_videos (platform, platform_id, platform_url, published_at, linked_file_id, match_status)
        VALUES (?, ?, ?, ?, ?, ?)
      `).run(data.platform, data.platform_id, data.platform_url ?? null, publishedAt, linkedFileId, matchStatus);
    } else {
      db.prepare(`
