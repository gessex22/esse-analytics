...
> **Actualización 2026-09-01:** la reconciliación histórica de `content_id` y
> los índices únicos parciales de `files`/`backup_files` sí quedaron aplicados
> el 2026-08-31; el detalle y postflight están en
> `docs/SYNC-01-audit-2026-08-30.md`. Lo que sigue pendiente es otro trabajo:
> consolidar físicamente colecciones redundantes, empezando por evaluar
> `files` + `backup_files`, sin asumir que todos los mirrors/historiales tienen
> la misma semántica. El alcance está anotado en `docs/product-backlog.md`.
...
| P1 — identidad de `files`/`backup_files` por `content_id` | ✅ Reconciliada y blindada (2026-08-31) | Reconciliados 1,033/1,033 registros seguros; el único ambiguo quedó excluido. Postflight independiente: 0 duplicados. Creados índices únicos parciales `{userId,content_id}` en ambas colecciones. Ver el estado final de SYNC-02 #1 en `docs/SYNC-01-audit-2026-08-30.md`. |
| P1 — consolidación física de colecciones | ⏳ Pendiente | La identidad ya converge, pero `files`/`backup_files` siguen siendo colecciones distintas con lectores y escritores en varios clientes. Diseñar migración compatible y reversible antes de fusionarlas; evaluar después las familias de publicaciones, historial y transcripciones. Ver `docs/product-backlog.md`. |
