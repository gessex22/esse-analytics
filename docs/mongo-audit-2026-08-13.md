...
| P1 — identidad de `files`/`backup_files` por `content_id` | 🚫 **Sigue bloqueado por revisión independiente** | Ver `docs/mongo-remediation-review-2026-08-13.md`. El fix tal como estaba planteado no soluciona el problema de identidad y agrega un modo de falla nuevo (`content_id` no es estable, lo reescribe el propio escritor). Requiere los fixes de código C1-C3 + pruebas de cliente reales antes de tocar este índice. |
