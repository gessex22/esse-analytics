...
| P1 — identidad de `files`/`backup_files`/`file_path` | 🚫 **Bloqueado por revisión independiente** | Ver `docs/mongo-remediation-review-2026-08-13.md`. El fix tal como estaba planteado no soluciona el problema de identidad y agrega un modo de falla nuevo (`content_id` no es estable, lo reescribe el propio escritor). Requiere 5 fixes de código previos (C1-C5) + pruebas de cliente reales antes de tocar índices. Sí encontró un bug activo real (`file_path` global ya colisiona, 55 placeholders) con un fix aditivo de bajo riesgo aparte. |
| P2 — normalización de arrays nulos / archivo con >3 resoluciones | ✅ Bug de enum arreglado; normalización de arrays sin aplicar | `file.model.ts::Platform` y los enums de `platforms`/`platforms_discarded` no incluían `'facebook'` (`platform-video.model.ts` sí lo tenía). Agregado. `remote_library_videos` tiene el mismo hueco pero está evitado a propósito en `backup.controller.ts:933` (filtra `facebook` antes de escribir ahí) — no se tocó. Los 19 arrays nulos siguen sin normalizar. |
