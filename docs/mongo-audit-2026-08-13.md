...
| P2 — normalización de arrays nulos / archivo con >3 resoluciones | 🚫 Supuesto falso encontrado | El "archivo con >3 resoluciones" NO es un dato corrupto — `facebook` es un destino legítimo de crossposting; el bug real es que falta en el enum de Mongoose. No aplicar el update a mano. Los 19 arrays nulos sí son seguros de normalizar. |
